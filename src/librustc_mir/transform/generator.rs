...
fn compute_layout<'tcx>(
    ...
    let dummy_local = LocalDecl::new_internal(tcx.mk_unit(), body_cache.span);
    ...
    for (idx, local) in live_locals.iter().enumerate() {
        let var = mem::replace(&mut body_cache.local_decls[local], dummy_local.clone());
        locals.push(local);
        tys.push(var.ty);
        decls.push(var);
...
fn create_generator_resume_function<'tcx>(
    ...
    cases.insert(1, (RETURNED, insert_panic_block(tcx, body_cache, GeneratorResumedAfterReturn)));
    // Panic when resumed on the poisoned state
    cases.insert(2, (POISONED, insert_panic_block(tcx, body_cache, GeneratorResumedAfterPanic)));
